...
function toAttendance(row: { id: string; sessionId: string; userId: string; createdAt: Date }) {
  return {
    id: row.id,
...
export async function listSessions(organizationId: string, rfpId?: string) {
  const rows = await tvetRepository.listSessionsByOrganization(organizationId, rfpId)
