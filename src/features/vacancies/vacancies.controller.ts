...
export const listPublic: AppRouteHandler<ListPublicVacanciesRoute> = async (
  c
) => {
...
export const listMine: AppRouteHandler<ListMineVacanciesRoute> = async (c) => {
  return c.json(
    await vacanciesService.listOrganizationVacancies(organizationId(c)),
    HttpStatusCodes.OK
  );
};
...
export const getMine: AppRouteHandler<GetMineVacancyRoute> = async (c) => {
  try {
    ...
    return c.json(
      await vacanciesService.getOwnVacancy(organizationId(c), id),
      HttpStatusCodes.OK
    );
  } catch (error) {
...
export const resubmit: AppRouteHandler<ResubmitVacancyRoute> = async (c) => {
  try {
    const { id } = c.req.valid("param");
    return c.json(
      await vacanciesService.resubmitOwnVacancy(organizationId(c), id),
      HttpStatusCodes.OK
    );
  } catch (error) {
    const mapped = asVacancyError(error);
...
export const get: AppRouteHandler<GetVacancyRoute> = async (c) => {
  try {
    const { id } = c.req.valid("param");
    return c.json(
      await vacanciesService.getPublicVacancy(id),
      HttpStatusCodes.OK
    );
  } catch (error) {
    const mapped = asVacancyError(error);
