import { type Workspace, isWorkspace } from "../../auth/access/workspace.ts";
...
export async function updateWorkspace(userId: string, workspace: Workspace) {
  ...
  if (existing.role !== "employer") {
    throw new WorkspaceError(
      400,
      "Workspace switch is only available on employer accounts"
    );
  }

  if (
    workspace === "training_provider" &&
    existing.hasTvetCapability !== true
  ) {
    throw new WorkspaceError(403, "TVET capability required");
  }

  const row = await usersRepository.updateUserById(userId, {
