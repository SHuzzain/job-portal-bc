...
export async function findByUserAndVacancy(userId: string, vacancyId: string) {
  const [row] = await db
    .select()
    .from(application)
    .where(and(eq(application.userId, userId), eq(application.vacancyId, vacancyId)))
    .limit(1)
  return row ?? null
}
...
export async function listByVacancyId(vacancyId: string) {
  return db.select().from(application).where(eq(application.vacancyId, vacancyId))
}

export async function findById(id: string) {
  const [row] = await db.select().from(application).where(eq(application.id, id)).limit(1)
  return row ?? null
}

export async function updateStatusById(id: string, status: string) {
  const [row] = await db.update(application).set({ status }).where(eq(application.id, id)).returning()
  return row ?? null
}
