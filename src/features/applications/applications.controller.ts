...
export const listMine: AppRouteHandler<ListMineRoute> = async (c) => {
  const session = authedSession(c);
  return c.json(
    await applicationsService.listMine(session.user.id),
    HttpStatusCodes.OK
  );
};

export const listForVacancy: AppRouteHandler<ListForVacancyRoute> = async (
  c
) => {
  try {
...
export const setStatus: AppRouteHandler<SetStatusRoute> = async (c) => {
  try {
    ...
    return c.json(
      await applicationsService.setStatus(organizationId(c), id, status),
      HttpStatusCodes.OK
    );
  } catch (error) {
    const mapped = mapError(error);
...
export const followUp: AppRouteHandler<FollowUpRoute> = async (c) => {
  try {
    const { id } = c.req.valid("param");
    return c.json(
      await applicationsService.requestFollowUp(organizationId(c), id),
      HttpStatusCodes.OK
    );
  } catch (error) {
    const mapped = mapError(error);
