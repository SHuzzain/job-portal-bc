...
import {
  completeLatestForApplication,
  latestInterviewMap,
} from "../interviews/interviews.service.ts";
...
function toApplication(
  ...
  interview: ReturnType<typeof latestInterviewMap> extends Map<string, infer T>
    ? T | null
    : never
) {
  return {
...
export async function apply(
  ...
  if (!isProfileComplete(profile)) {
    throw new ApplicationError(
      403,
      "Complete your 5-step profile before applying"
    );
  }
  ...
  const existing = await applicationsRepository.findByUserAndVacancy(
    userId,
    vacancyId
  );
  if (existing) {
...
export async function listForVacancy(
...
export async function setStatus(
  ...
  if (status !== "FAILED" && !next.includes(status)) {
    throw new ApplicationError(
      400,
      `Cannot move application from ${existing.status} to ${status}`
    );
  }
...
export async function requestFollowUp(organizationId: string, id: string) {
  ...
  return toApplication(
    existing,
    (await interviewsFor([existing.id])).get(existing.id) ?? null
  );
}
...
export async function markStaleForOrganization(organizationId: string) {
  ...
  const rows = await applicationsRepository.listStaleForOrganization(
    organizationId,
    cutoff
  );
