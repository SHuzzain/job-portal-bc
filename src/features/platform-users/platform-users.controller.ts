...
export const listPlatformUsers: AppRouteHandler<
  ListPlatformUsersRoute
> = async (c) => {
  return c.json(await service.listUsers(), HttpStatusCodes.OK);
};

export const createPlatformUser: AppRouteHandler<
  CreatePlatformUserRoute
> = async (c) => {
  try {
    return c.json(
...
export const assignPlatformRole: AppRouteHandler<
  AssignPlatformRoleRoute
> = async (c) => {
  try {
    return c.json(
...
export const updatePlatformUser: AppRouteHandler<
  UpdatePlatformUserRoute
> = async (c) => {
  try {
    return c.json(
