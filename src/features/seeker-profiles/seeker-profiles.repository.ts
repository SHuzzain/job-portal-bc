...
export async function findByUserId(userId: string) {
  const [row] = await db.select().from(seekerProfile).where(eq(seekerProfile.userId, userId)).limit(1)
  return row ?? null
}
...
export async function updateByUserId(userId: string, data: UpdateSeekerProfileRecord) {
  const [row] = await db
