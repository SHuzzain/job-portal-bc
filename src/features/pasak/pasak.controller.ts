...
export const setCompanyStatus: AppRouteHandler<SetCompanyStatusRoute> = async (c) => {
    try {
      const { id } = c.req.valid("param")
      const { status } = c.req.valid("json")
      return c.json(await pasakService.setCompanyStatus(id, status), HttpStatusCodes.OK)
    } catch (error) {
...
export const reviewCompany: AppRouteHandler<ReviewCompanyRoute> = async (c) => {
    try {
      ...
      return c.json(await pasakService.reviewCompany(id, action, comments), HttpStatusCodes.OK)
    } catch (error) {
      const mapped = pasakError(error)
      return c.json({ message: mapped.message }, mapped.status)
    }
...
export const reviewVacancy: AppRouteHandler<ReviewVacancyRoute> = async (c) => {
    try {
      const { id } = c.req.valid("param")
      const { action, comments } = c.req.valid("json")
      return c.json(await pasakService.reviewVacancy(id, action, comments), HttpStatusCodes.OK)
    } catch (error) {
...
export const setVacancyStatus: AppRouteHandler<SetVacancyStatusRoute> = async (c) => {
    try {
      const { id } = c.req.valid("param")
      const { status } = c.req.valid("json")
      return c.json(await pasakService.setVacancyStatus(id, status), HttpStatusCodes.OK)
    } catch (error) {
      if (error instanceof VacancyError) {
        return c.json({ message: error.message }, HttpStatusCodes.NOT_FOUND)
      }
...
export const setTvetCapability: AppRouteHandler<SetTvetCapabilityRoute> = async (c) => {
    try {
      const { id } = c.req.valid("param")
      const { hasTvetCapability } = c.req.valid("json")
      return c.json(await pasakService.setTvetCapability(id, hasTvetCapability), HttpStatusCodes.OK)
    } catch (error) {
      const mapped = pasakError(error)
      return c.json({ message: mapped.message }, mapped.status)
    }
