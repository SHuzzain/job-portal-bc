...
export async function reviewCompany(
  ...
  const notes =
    action === "APPROVE" ? null : comments?.trim() || existing.reviewNotes;
...
export async function setTvetCapability(
  id: string,
  ...
  if (existing.role !== "employer") {
    throw new PasakError(
      400,
      "TVET capability can only be set on employer accounts"
    );
  }
